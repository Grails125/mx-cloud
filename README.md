...
- **本地开发**: 直接调用云服务商 API（可能遇到 CORS 问题，需要使用浏览器扩展或代理）
- **Vercel 部署**: 自动使用 Serverless Function 代理，避免 CORS 跨域问题
- 项目已内置 CORS 代理解决方案，部署到 Vercel 后无需额外配置

## ⚠️ 注意事项

1. **跨域问题（CORS）**: 
   - ✅ **已解决**: 项目已内置 Vercel Serverless Function 代理 (`api/proxy.ts`)
   - ✅ **自动启用**: 部署到 Vercel 后自动使用代理，无需配置
   - ⚠️ **本地开发**: 如果遇到 CORS 问题，可以使用：
     - 浏览器扩展（如 CORS Unblock）
     - 本地代理服务器
     - 或直接部署到 Vercel 测试
